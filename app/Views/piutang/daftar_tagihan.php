...
<div class="card shadow-none position-relative overflow-hidden mb-4">
    <div class="card-body d-flex align-items-center justify-content-between p-4">
        <h4 class="fw-semibold mb-0">Daftar Piutang</h4>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0">
                <li class="breadcrumb-item">
                    <a class="text-muted text-decoration-none" href="<?= base_url('/') ?>">Daftar</a>
                </li>
...
<div class="card w-100 position-relative overflow-hidden">
    ...
    <form action="<?= base_url('export_daftar_piutang') ?>" method="post" enctype="multipart/form-data">
                        ...
                        <tr>
                            ...
                            <td>
                                <button type="button" class="btn btn-warning edit-button" style="display: flex; gap: 5px; justify-content: center; padding-top: 16px;" data-bs-toggle="modal"
                                    ...
                                    data-idpiutang="<?= esc($row->idpiutang) ?>"
                                    data-tanggal="<?= esc($row->tanggal) ?>"
                                    data-sisa="<?= esc($row->sisa_hutang) ?>">
...
<div class="modal fade" id="bayar-tagihan-modal" tabindex="-1" aria-labelledby="bayarTagihanModalLabel"
    ...
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            ...
            <form action="<?= base_url('update_cicilan_piutang') ?>" enctype="multipart/form-data" method="post">
                <div class="modal-body">
                    <!-- ID Pembelian -->
                    <input type="hidden" id="edit_idpiutang" name="idpiutang">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelector('#zero_config').addEventListener('click', function(e) {
            if (e.target.closest('.edit-button')) {
                ...
                const idpiutang = button.getAttribute('data-idpiutang');

                const tanggal_masuk = button.getAttribute('data-tanggal');
                const sisaAwal = button.getAttribute('data-sisa'); // ambil sisa dari tombol

                document.getElementById('edit_idpiutang').value = idpiutang;
